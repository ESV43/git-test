...
Produce the complete screenplay following these instructions precisely.`;
  ...
  private async callGeminiTextAPI(prompt: string): Promise<string> {
    return this._callGeminiWithRetry(async (apiKey) => {
      const genAI = new GoogleGenAI({ apiKey });
      const chat = genAI.chats.create({ model: 'gemini-2.0-flash-preview', history: [] });
  ...
  async generateCharacterPreview(refImg: string, desc: string, style?: string): Promise<string> {
    return this._callGeminiWithRetry(async (apiKey) => {
      ...
      const chat = genAI.chats.create({
        model: 'gemini-2.0-flash-preview-image-generation',
        config: { responseModalities: [Modality.TEXT, Modality.IMAGE] }, history: [],
      });
      ...
      const result = await chat.sendMessageStream({ message: [{ text: prompt }, { inlineData: { data: refImg.split(',')[1], mimeType: 'image/png' } }] });
      for await (const chunk of result) {
        for (const c of chunk.candidates) for (const p of c.content.parts ?? []) if (p.inlineData?.data) return `data:image/png;base64,${p.inlineData.data}`;
      }
